...
def update_vcf_from_grmpy(inVcfFilename, grmpyOutput, outVcfFilename):
    variantFile = pysam.VariantFile(inVcfFilename)

    # pylint: disable=no-member
    header = variantFile.header
    ...
    if 'AD' not in header.formats:
        header.add_line('##FORMAT=<ID=AD,Number=R,Type=Integer,Description="Allele depth for each allele, including the reference.">')
    if 'ADF' not in header.formats:
        header.add_line('##FORMAT=<ID=ADF,Number=R,Type=Integer,Description="Allele depth on forward strand for each allele, including the reference.">')
    if 'ADR' not in header.formats:
        header.add_line('##FORMAT=<ID=ADR,Number=R,Type=Integer,Description="Allele depth on reverse strand for each allele, including the reference.">')
    ...
    header.add_line('##FILTER=<ID=ALL_BAD_BP,Description="All paragraph breakpoints were problematic">')
    header.add_line('##FILTER=<ID=EXIST_BAD_BP,Description="One breakpoint was found to be problematic">')
    header.add_line('##FILTER=<ID=CONFLICT,Description="Breakpoints gave different genotypes">')
    header.add_line('##FILTER=<ID=MISSING,Description="One genotype was missing">')
    header.add_line('##FILTER=<ID=NO_READS,Description="No reads could be retrieved for a breakpoint.">')
    header.add_line('##FILTER=<ID=DEPTH,Description="Poisson depth filter: observed depth deviates too far from Poisson expectation">')
    ...
    for record in variantFile:
        ...
        try:
            grmpyRecords = [grmpyOutput["by_id"][record.info["GRMPY_ID"]]]
        except KeyError:
            grmpyRecords = []

        if not grmpyRecords:
            # match by sequence name if we don't have GRMPY_ID
            grmpyRecords = [grmpyOutput["by_sequencename"][aId] for aId in alleleIds if aId in grmpyOutput["by_sequencename"]]
        ...
        for sample in record.samples:
            old_gt = "/".join(sorted([str(val) if val is not None else "." for val in record.samples[sample]["GT"]]))
    ...
    logging.info(f"VCF to graph matching statistics: {matched} matched, {unmatched} unmatched, {multimatched} multiple matches.")
